...
import { StadiumRouteProvider } from '@/app/contexts/StadiumRouteContext';
...
describe('StadiumMockup', () => {
  ...
  const renderWithProvider = (contextValue) => {
    return render(
      <StadiumRouteProvider value={contextValue}>
        <StadiumMockup />
  ...
  it('renders stadium mockup and toggles day/night', async () => {
    ...
    renderWithProvider({ ticket: mockTicket, routeMode: 'hide', setRouteMode: jest.fn(), selectedAmenityId: 'Pizza', setSelectedAmenityId: jest.fn(), amenityFilter: 'All', setAmenityFilter: jest.fn(), update: jest.fn() });
  ...
  it('calls setRouteMode when route dropdown changes', async () => {
    ...
    const user = userEvent.setup();
    renderWithProvider({ ticket: mockTicket, routeMode: 'hide', setRouteMode, selectedAmenityId: 'Pizza', setSelectedAmenityId: jest.fn(), amenityFilter: 'All', setAmenityFilter: jest.fn(), update: jest.fn() });
  ...
  it('calls setAmenityFilter when amenity filter changes', async () => {
    ...
    const user = userEvent.setup();
    renderWithProvider({ ticket: mockTicket, routeMode: 'hide', setRouteMode: jest.fn(), selectedAmenityId: 'Pizza', setSelectedAmenityId: jest.fn(), amenityFilter: 'All', setAmenityFilter, update: jest.fn() });
  ...
  it('handles gate selection', async () => {
    ...
    const user = userEvent.setup();
    renderWithProvider({ ticket: mockTicket, routeMode: 'hide', setRouteMode: jest.fn(), selectedAmenityId: 'Pizza', setSelectedAmenityId: jest.fn(), amenityFilter: 'All', setAmenityFilter: jest.fn(), update });

    const gateB = screen.getByLabelText('Select Gate B');
    await user.click(gateB);
    expect(update).toHaveBeenCalledWith('gate', 'Gate B');
  ...
  it('renders different route modes', () => {
    const { rerender } = renderWithProvider({ ticket: mockTicket, routeMode: 'gate-seat', setRouteMode: jest.fn(), selectedAmenityId: 'Pizza', setSelectedAmenityId: jest.fn(), amenityFilter: 'All', setAmenityFilter: jest.fn(), update: jest.fn() });
    expect(screen.getByText('SEAT A-12')).toBeInTheDocument();

    rerender(
      <StadiumRouteProvider value={{ ticket: mockTicket, routeMode: 'gate-amenity', setRouteMode: jest.fn(), selectedAmenityId: 'Pizza', setSelectedAmenityId: jest.fn(), amenityFilter: 'All', setAmenityFilter: jest.fn(), update: jest.fn() }}>
        <StadiumMockup />
      </StadiumRouteProvider>
    );
    rerender(
      <StadiumRouteProvider value={{ ticket: mockTicket, routeMode: 'gate-seat-amenity', setRouteMode: jest.fn(), selectedAmenityId: 'Pizza', setSelectedAmenityId: jest.fn(), amenityFilter: 'All', setAmenityFilter: jest.fn(), update: jest.fn() }}>
        <StadiumMockup />
      </StadiumRouteProvider>
    );
  });
  ...
  it('handles amenity clicks and keydowns', async () => {
    ...
    renderWithProvider({ ticket: mockTicket, routeMode: 'hide', setRouteMode, selectedAmenityId: 'Pizza', setSelectedAmenityId, amenityFilter: 'All', setAmenityFilter: jest.fn(), update: jest.fn() });
